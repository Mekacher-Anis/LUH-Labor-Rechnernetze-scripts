...
sudo tc filter add dev $3 parent 1:0 prio 2 protocol ip u32 match ip dport 30001 0xffff match ip dst 10.1.0.3/32 flowid 1:11 &&
sudo tc filter add dev $3 parent 1:0 prio 2 protocol ip u32 match ip dport 30002 0xffff match ip dst 10.1.0.3/32 flowid 1:12 &&
sudo tc filter add dev $3 parent 1:0 prio 2 protocol ip u32 match ip dport 30003 0xffff match ip dst 10.1.0.3/32 flowid 1:13 &&
sudo tc filter add dev $3 parent 1:0 prio 2 protocol ip u32 match ip dport 30004 0xffff match ip dst 10.1.0.3/32 flowid 1:13 &&
sudo tc filter add dev $3 parent 1:0 prio 2 protocol ip u32 match ip dport 30001 0xffff match ip dst 10.1.0.4/32 flowid 1:21 &&
sudo tc filter add dev $3 parent 1:0 prio 2 protocol ip u32 match ip dport 30002 0xffff match ip dst 10.1.0.4/32 flowid 1:22 &&
sudo tc filter add dev $3 parent 1:0 prio 2 protocol ip u32 match ip dport 30003 0xffff match ip dst 10.1.0.4/32 flowid 1:23 &&
sudo tc filter add dev $3 parent 1:0 prio 2 protocol ip u32 match ip dport 30004 0xffff match ip dst 10.1.0.4/32 flowid 1:23
EOF
)
